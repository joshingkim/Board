...
function uploadedItemForRead(filename){
	...
	var msg =`
		<div class="uploadeditem col-sm-3">
			...
			<p><a target="_blank" href="/test/downloadorshow?linkurl=${getLinkUrl(filename)}">${originalFilename}</a></p>
		</div>
	`;
	return msg;
}
...
function checkImageFile2(filename){
	...
	if(formatName == "PNG" || formatName == "JPG" || formatName == "JPEG" || formatName == "GIF"){
		return true;
	}else{
		return false;
	}
...
function getRepliesPage(bno, curPage, el){
	...
	$.getJSON("/replies/"+bno+"/"+curPage, function(data){
		...
		for(var i=0;i<arr.length;i++){
			...
			msg = replyform(obj["replyer"], obj["updatedate"], obj["reply"], obj["rno"], data['curPage']);
			el.append(msg);
		}
...
function replyform(replyer, updatedate, reply, rno, curPage){
	...
	var msg = `
					      ...
					      <div class="card-body">
					         ...
					         <a data-replyer="${replyer}" data-rno="${rno}" data-curPage="${curPage}" href="#" class="btn btn-warning btn-sm btn_reply_updateui_form">수정</a> 
					         <a data-rno="${rno}" data-curPage="${curPage}" href="#" class="btn btn-danger btn-sm btn_reply_delete">삭제</a>
